...
contract('NormalSale', function ([deployer, investor, wallet, purchaser, purchaser2, purchaser3, purchaser4]) {
  ...
  describe('creating a valid crowdsale', function () {
    it('should fail with zero rate', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, 0, softCap, softCapTime, cap, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with zero cap', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, softCap, softCapTime, 0, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with greater softCap than cap', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, cap, softCapTime, softCap, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with zero controller', async function () {
      await Crowdsale.new(0, this.startTime, this.endTime, minContribution, rate, softCap, softCapTime, cap, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with zero wallet', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, softCap, softCapTime, cap, maxGasPrice, maxGasPenalty, 0).should.be.rejectedWith(EVMRevert);
    });
  });
  ...
  describe('modify before sale', function () {
    ...
    it('should fail to set valid caps after start', async function () {
      ...
      await this.crowdsale.setCaps(softCap, softCapTime, cap).should.be.rejectedWith(EVMRevert);
    });

    it('should fail setting zero cap', async function () {
      await this.crowdsale.setCaps(softCap, softCapTime, 0).should.be.rejectedWith(EVMRevert);
    });

    it('should fail setting greater softCap than cap', async function () {
      await this.crowdsale.setCaps(cap, softCapTime, softCap).should.be.rejectedWith(EVMRevert);
    });
