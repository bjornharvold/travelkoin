...
const Crowdsale = artifacts.require('TravelkoinPreSale');
const TravelkoinHodler = artifacts.require('TravelkoinHodler');
const Vault = artifacts.require('TravelkoinRefundVault');
...
contract('PreSale', function ([deployer, investor, wallet, purchaser, purchaser2, purchaser3, purchaser4]) {
  ...
  describe('creating a valid crowdsale', function () {
    it('should fail with zero rate', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, 0, goal, softCap, softCapTime, cap, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with zero goal', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, 0, softCap, softCapTime, cap, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with zero softCap', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, goal, 0, softCapTime, cap, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with zero cap', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, goal, softCap, softCapTime, 0, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with greater goal than softCap', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, cap, softCap, softCapTime, goal, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with greater softCap than cap', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, goal, cap, softCapTime, softCap, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with zero controller', async function () {
      await Crowdsale.new(0, this.startTime, this.endTime, minContribution, rate, goal, softCap, softCapTime, cap, maxGasPrice, maxGasPenalty, wallet).should.be.rejectedWith(EVMRevert);
    });

    it('should fail with zero wallet', async function () {
      await Crowdsale.new(this.controller.address, this.startTime, this.endTime, minContribution, rate, goal, softCap, softCapTime, cap, maxGasPrice, maxGasPenalty, 0).should.be.rejectedWith(EVMRevert);
    });
  });
  ...
  describe('modify before sale', function () {
    ...
    it('should fail to set valid caps after start', async function () {
      ...
      await this.crowdsale.setCaps(goal, softCap, softCapTime, cap).should.be.rejectedWith(EVMRevert);
    });

    it('should fail setting zero goal', async function () {
      await this.crowdsale.setCaps(0, softCap, softCapTime, cap).should.be.rejectedWith(EVMRevert);
    });

    it('should fail setting zero softCap', async function () {
      await this.crowdsale.setCaps(goal, 0, softCapTime, cap).should.be.rejectedWith(EVMRevert);
    });

    it('should fail setting zero cap', async function () {
      await this.crowdsale.setCaps(goal, softCap, softCapTime, 0).should.be.rejectedWith(EVMRevert);
    });

    it('should fail setting greater goal than softCap', async function () {
      await this.crowdsale.setCaps(softCap, goal, softCapTime, cap).should.be.rejectedWith(EVMRevert);
    });

    it('should fail setting greater softCap than cap', async function () {
      await this.crowdsale.setCaps(goal, cap, softCapTime, softCap).should.be.rejectedWith(EVMRevert);
    });
  ...
  describe('refund', function () {
    ...
    it('should be able to extract tokens from vault after sale', async function () {
      ...
      await this.crowdsale.extractVaultTokens(this.token.address, wallet).should.be.rejectedWith(EVMRevert);
  ...
  describe('claim token', function () {
    ...
    it('should allow token transfer after controller unpaused', async function () {
      ...
      await this.token.transfer(purchaser, goal.mul(rate), { from: investor, gasPrice: 0 }).should.be.rejectedWith(EVMRevert);
