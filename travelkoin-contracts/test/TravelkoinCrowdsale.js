...
contract('TravelkoinCrowdsale', function ([deployer, investor, wallet, purchaser, purchaser2, purchaser3, purchaser4]) {
    ...
    beforeEach(async function () {
        ...
        this.startTimePlusOneDay = this.startTime + duration.days(1);
        this.endTime = this.startTime + duration.weeks(4);
        this.afterEndTime = this.endTime + duration.seconds(1);

        this.token = await Token.new(tokenName, tokenSymbol, decimals);
        this.crowdsale = await Crowdsale.new(this.startTime, this.endTime, minContribution, dayOneMaxContribution, rate, cap, wallet, this.token.address);
    });

    describe('creating a valid crowdsale', function () {
      it('should fail with zero rate', async function () {
        await Crowdsale.new(this.startTime, this.endTime, minContribution, dayOneMaxContribution, 0, cap, wallet, this.token.address).should.be.rejectedWith(EVMRevert);
      });

      it('should fail with zero cap', async function () {
        await Crowdsale.new(this.startTime, this.endTime, minContribution, dayOneMaxContribution, rate, 0, wallet, this.token.address).should.be.rejectedWith(EVMRevert);
      });

      it('should fail with zero wallet', async function () {
        await Crowdsale.new(this.startTime, this.endTime, minContribution, dayOneMaxContribution, rate, cap, 0, this.token.address).should.be.rejectedWith(EVMRevert);
      });

      it('should fail with zero token', async function () {
        await Crowdsale.new(this.startTime, this.endTime, minContribution, dayOneMaxContribution, rate, cap, wallet, 0).should.be.rejectedWith(EVMRevert);
      });
    });
    ...
    describe('accepting payments', function () {
        ...
        it('should accept payments after start if whitelisted', async function () {
          await this.crowdsale.addManyToWhitelist([investor, purchaser]).should.be.fulfilled;
          await increaseTimeTo(this.startTime);
        ...
        it('should reject payments outside cap', async function () {
          await this.crowdsale.addManyToWhitelist([investor, purchaser2]).should.be.fulfilled
          await increaseTimeTo(this.startTimePlusOneDay);

          await this.crowdsale.send(cap).should.be.rejectedWith(EVMRevert);
          await this.crowdsale.buyTokens(investor, { value: cap, from: purchaser2 }).should.be.fulfilled;
          await this.crowdsale.buyTokens(investor, { value: cap, from: purchaser2 }).should.be.rejectedWith(EVMRevert);
        });
    ...
    describe('high-level purchase', function () {
      ...
      it('should log purchase', async function () {
      	await this.crowdsale.addManyToWhitelist([investor, purchaser2]).should.be.fulfilled
        const { logs } = await this.crowdsale.buyTokens(investor, { value: minContribution, from: purchaser2 });

        const event = logs.find(e => e.event === 'TokenPurchase');

        should.exist(event);
        event.args.purchaser.should.equal(purchaser2);
        event.args.beneficiary.should.equal(investor);
        event.args.value.should.be.bignumber.equal(minContribution);
        event.args.amount.should.be.bignumber.equal(minContribution.mul(rate));
      });
