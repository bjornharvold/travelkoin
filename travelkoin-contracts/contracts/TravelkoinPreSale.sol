...
import "./TravelkoinRefundableCrowdsale.sol";


/**
 * @title TravelkoinPreSale
 * @author Bjorn Harvold
 * @notice Travelkoin Token Sale round one presale contract, with mincap (goal), softcap and hardcap (cap)
 * @dev This contract has to be finalized before refund or token claims are enabled
 */
contract TravelkoinPreSale is Pausable, TravelkoinCappedCrowdsale, TravelkoinRefundableCrowdsale {
    // the token is here
    ...
    TravelkoinRefundableCrowdsale(_goal)
    ...
    function claimTokenFor(address _beneficiary) public afterSaleSuccess whenNotPaused {
        ...
        // decrease tokenBalance, to make it possible to withdraw excess TKT funds
    ...
    function howMuchCanIContributeNow() public payable returns (uint256) {
    ...
    function transferToken(address _beneficiary, uint256 _weiAmount) internal {
        ...
        // calculate tokens, so we can refund excess tokens to TravelkoinController after token sale
    ...
    function finalization() internal {
        uint256 _balance = getTravelkoinBalance();
        ...
        if (goalReached()) {
            ...
            // send back the excess token to travelkoinController
            if (_balance > tokenBalance) {
                travelkoinController.token().transfer(travelkoinController.SALE(), _balance.sub(tokenBalance));
            }
        } else if (!goalReached() && _balance > 0) {
            // if token sale is failed, then send back all tokens to travelkoinController's sale address
            tokenBalance = 0;
            travelkoinController.token().transfer(travelkoinController.SALE(), _balance);
        }
