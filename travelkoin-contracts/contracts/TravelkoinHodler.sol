...
contract TravelkoinHodler is Ownable {
    ...
    function TravelkoinHodler(uint256 _stake3m, uint256 _stake6m, uint256 _stake9m) public {
    ...
    /// @dev Gets called by TravelkoinController#onTransfer before every transaction
    ...
    function claimHodlRewardFor(address _beneficiary) public {
        ...
        if (_stake > 0) {
            ...
            // transferring tokens
            require(TokenController(owner).token().transfer(_beneficiary, _stake));
    ...
    function updateAndGetHodlTotalValue() public returns (uint) {
        ...
        if (now >= hodlerTime9M && hodlerTotalValue9M == 0) {
            ...
            TOKEN_HODL_9M = TokenController(owner).token().balanceOf(this).sub(TOKEN_HODL_3M).sub(TOKEN_HODL_6M).add(claimedTokens);
        }
