...
/**
 * TravelkoinMultiSigWalletWithDailyLimit API
 */
export class TravelkoinMultiSigWalletWithDailyLimit extends SoltsiceContract {
    static get Artifacts() { return require('../contracts/TravelkoinMultiSigWalletWithDailyLimit.json'); }

    static get BytecodeHash() {
        // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
        let artifacts = TravelkoinMultiSigWalletWithDailyLimit.Artifacts;
    ...
    static async New(deploymentParams: W3.TX.TxParams, ctorParams?: {_owners: string[], _required: BigNumber | number, _dailyLimit: BigNumber | number}, w3?: W3, link?: SoltsiceContract[]): Promise<TravelkoinMultiSigWalletWithDailyLimit> {
        let contract = new TravelkoinMultiSigWalletWithDailyLimit(deploymentParams, ctorParams, w3, link);
        await contract._instancePromise;
        return contract;
    }

    static async At(address: string | object, w3?: W3): Promise<TravelkoinMultiSigWalletWithDailyLimit> {
        let contract = new TravelkoinMultiSigWalletWithDailyLimit(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    static async Deployed(w3?: W3): Promise<TravelkoinMultiSigWalletWithDailyLimit> {
        let contract = new TravelkoinMultiSigWalletWithDailyLimit('', undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }
    ...
    protected constructor(
        ...
        super(
            w3,
            TravelkoinMultiSigWalletWithDailyLimit.Artifacts,
