...
export class TravelkoinMiniMeToken extends SoltsiceContract {
    static get Artifacts() { return require('../contracts/TravelkoinMiniMeToken.json'); }
    ...
    static async New(deploymentParams: W3.TX.TxParams, ctorParams?: {_controller: string, _tokenFactory: string}, w3?: W3, link?: SoltsiceContract[]): Promise<TravelkoinMiniMeToken> {
        let contract = new TravelkoinMiniMeToken(deploymentParams, ctorParams, w3, link);
        await contract._instancePromise;
        return contract;
    }

    static async At(address: string | object, w3?: W3): Promise<TravelkoinMiniMeToken> {
        let contract = new TravelkoinMiniMeToken(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    static async Deployed(w3?: W3): Promise<TravelkoinMiniMeToken> {
