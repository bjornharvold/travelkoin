...
export class MiniMeToken extends SoltsiceContract {
    public static get Artifacts() { return require('../contracts/MiniMeToken.json'); }
    ...
    public static async New(deploymentParams: W3.TX.TxParams, ctorParams?: {_tokenFactory: string, _parentToken: string, _parentSnapShotBlock: BigNumber | number, _tokenName: string, _decimalUnits: BigNumber | number, _tokenSymbol: string, _transfersEnabled: boolean}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<MiniMeToken> {
        w3 = w3 || W3.Default;
        if (!privateKey) {
            ...
            let data = MiniMeToken.NewData(ctorParams, w3);
            let txHash = await w3.sendSignedTransaction(W3.zeroAddress, privateKey, data, deploymentParams);
            let txReceipt = await w3.waitTransactionReceipt(txHash);
            let rawAddress = txReceipt.contractAddress;
            let contract = await MiniMeToken.At(rawAddress, w3);
            return contract;
        }
    }

    public static async At(address: string | object, w3?: W3): Promise<MiniMeToken> {
        let contract = new MiniMeToken(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    public static async Deployed(w3?: W3): Promise<MiniMeToken> {
    ...
    // tslint:disable-next-line:max-line-length
    public static NewData(ctorParams?: {_tokenFactory: string, _parentToken: string, _parentSnapShotBlock: BigNumber | number, _tokenName: string, _decimalUnits: BigNumber | number, _tokenSymbol: string, _transfersEnabled: boolean}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.NewDataImpl(w3, MiniMeToken.Artifacts, ctorParams ? [ctorParams!._tokenFactory, ctorParams!._parentToken, ctorParams!._parentSnapShotBlock, ctorParams!._tokenName, ctorParams!._decimalUnits, ctorParams!._tokenSymbol, ctorParams!._transfersEnabled] : []);
        return data;
    }
