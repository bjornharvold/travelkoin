 ...
 * DetailedERC20 API
 */
export class DetailedERC20 extends SoltsiceContract {
    public static get artifacts() { return require('../contracts/DetailedERC20.json'); }

    public static get bytecodeHash() {
        // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
        let artifacts = DetailedERC20.artifacts;
    ...
    public static async new(deploymentParams: W3.TX.TxParams, ctorParams?: {_name: string, _symbol: string, _decimals: BigNumber | number}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<DetailedERC20> {
        w3 = w3 || W3.default;
        if (!privateKey) {
            let contract = new DetailedERC20(deploymentParams, ctorParams, w3, link);
            await contract._instancePromise;
            return contract;
        } else {
            let data = DetailedERC20.newData(ctorParams, w3);
            let txHash = await w3.sendSignedTransaction(W3.zeroAddress, privateKey, data, deploymentParams);
            let txReceipt = await w3.waitTransactionReceipt(txHash);
            let rawAddress = txReceipt.contractAddress;
            let contract = await DetailedERC20.at(rawAddress, w3);
            return contract;
        }
    }

    public static async at(address: string | object, w3?: W3): Promise<DetailedERC20> {
        let contract = new DetailedERC20(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    public static async deployed(w3?: W3): Promise<DetailedERC20> {
        let contract = new DetailedERC20('', undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    // tslint:disable-next-line:max-line-length
    public static newData(ctorParams?: {_name: string, _symbol: string, _decimals: BigNumber | number}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.newDataImpl(w3, DetailedERC20.artifacts, ctorParams ? [ctorParams!._name, ctorParams!._symbol, ctorParams!._decimals] : []);
        return data;
    }

    protected constructor(
        deploymentParams: string | W3.TX.TxParams | object,
        ctorParams?: {_name: string, _symbol: string, _decimals: BigNumber | number},
        w3?: W3,
        ...
        super(
            w3,
            DetailedERC20.artifacts,
            ctorParams ? [ctorParams!._name, ctorParams!._symbol, ctorParams!._decimals] : [],
            deploymentParams,
            link
        );
    }
    ...
    public approve = Object.assign(
        ...
        },
        {
            ...
            estimateGas: (spender: string, value: BigNumber | number): Promise<number> => {
                return new Promise((resolve, reject) => {
                    this._instance.approve.estimateGas(spender, value).then((g: any) => resolve(g));
                });
            }
        });
    ...
    public transferFrom = Object.assign(
        ...
        },
        {
            ...
            estimateGas: (from: string, to: string, value: BigNumber | number): Promise<number> => {
                return new Promise((resolve, reject) => {
                    this._instance.transferFrom.estimateGas(from, to, value).then((g: any) => resolve(g));
                });
            }
        });
    ...
    public transfer = Object.assign(
        ...
        },
        {
            ...
            estimateGas: (to: string, value: BigNumber | number): Promise<number> => {
                return new Promise((resolve, reject) => {
                    this._instance.transfer.estimateGas(to, value).then((g: any) => resolve(g));
                });
            }
        });
